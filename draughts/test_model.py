...
def speeds():
    """Get a rough number on how long some operations and their alternatives take."""
    ...
    return {
        ...
        'construct_preserve': [
            None,
            timeit('Test(data); assert data["a"] == 1111', globals={'Test': MTest, 'data': dict(input_object)}, number=number),
            # None, # timeit('Test(data); assert data["a"] == 1111', globals={'Test': STest, 'data': dict(input_object)}, number=number),
            timeit('Test(data); assert data["a"] == 1111', globals={'Test': PTest, 'data': dict(input_object)}, number=number),
        ],
        'write_int': [
        ...
        'read property': [
            None,
            timeit('x.c', globals={'x': m_x, 'string': string}, number=number),
            # None,  # timeit('x.c', globals={'x': s_x, 'string': string}, number=number),
            timeit('x.c', globals={'x': p_x, 'string': string}, number=number),
        ],
        'write property': [
            None,
            timeit('x.c = string', globals={'x': m_x, 'string': '100'}, number=number),
            # None,  # timeit('x.c = string', globals={'x': s_x, 'string': '100'}, number=number),
            timeit('x.c = string', globals={'x': p_x, 'string': '100'}, number=number),
        ],
        'call method': [
            None,
            timeit('x.bobulate()', globals={'x': m_x}, number=number),
            # None,  # timeit('x.c = string', globals={'x': s_x, 'string': '100'}, number=number),
            timeit('x.bobulate()', globals={'x': p_x}, number=number),
        ],
    },
...
def nested_speeds():
    ...
    class MInner:
        a: int = model.Integer()
        b: str = model.List(model.String())

    @model.model
    class MTest:
        a = model.Compound(MInner)
        b = model.List(model.Compound(MInner))

    @proxy_model.model
    class PInner:
        a: int = proxy_model.Integer()
        b: str = proxy_model.List(proxy_model.String())

    @proxy_model.model
    class PTest:
        a = proxy_model.Compound(PInner)
        b = proxy_model.List(proxy_model.Compound(PInner))

    m_x = MTest(a=dict(a=10, b=["100!", "cat"]), b=[dict(a=10, b=["100!", "cat"]), dict(a=999, b=["rat", "dog"])])
    # s_x = STest(a=10, b="x"*1000)
    p_x = PTest(a=dict(a=10, b=["100!", "cat"]), b=[dict(a=10, b=["100!", "cat"]), dict(a=999, b=["rat", "dog"])])
    # e = TestEnvelope(a=x)
    # string = 'y'*2990
    # input_object = {"a": '1111', "b": "string"}
    #
